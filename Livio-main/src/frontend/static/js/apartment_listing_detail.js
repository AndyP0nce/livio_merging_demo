...
class ListingModal {
  ...
  _bindEvents() {
    ...
    document.addEventListener('keydown', function(e) {
      ...
      if (e.key === 'ArrowLeft')  { self._currentSlide = (self._currentSlide - 1 + self._slides.length) % self._slides.length; self._updateSlide(); }
      if (e.key === 'ArrowRight') { self._currentSlide = (self._currentSlide + 1) % self._slides.length; self._updateSlide(); }
    });
