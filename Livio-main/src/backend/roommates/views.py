...
def currentUserRoommatePost(request):
    ...
    try:
        ...
        return Response({"detail": "No roommate post found for this user"}, status=status.HTTP_404_NOT_FOUND)
...
def createRoommatePost(request):
    ...
    if hasattr(request.user, 'profile') and hasattr(request.user.profile, 'has_roommate_post') and request.user.profile.has_roommate_post:
        return Response({"detail": "You already have a roommate post"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if serializer.is_valid():
        ...
        # change the has_roomate_post from false to true whenevr the user makes a roommate post
        profile = request.user.profile 
        profile.has_roommate_post = True
        profile.save()
...
def editRoommatePost(request):
    ...
    roommatePostGetSerializer = RoommatePostGetSerializer(current_profile, many=False)


    return Response(roommatePostGetSerializer.data, status=status.HTTP_200_OK)
...
def addInterestedRoommate(request):
    ...
    # add to the number of people interested in that profiles roommate post so it shows in the frontend
    interested_profile_roommate_post = RoommatePost.objects.get(profile=interested_profile) # gets the interested profile's roommate post
    interested_profile_roommate_post.numberOfPeopleInterested = interested_profile_roommate_post.numberOfPeopleInterested  + 1 # add 1 to the amount of people which are interested in the profile's rommate post, since this person is adding them to their profile
    interested_profile_roommate_post.save()  # save the db once the changes are made
...
def pagination(request):
    ...
    limit = int(query_params.get('limit', 10))  # Default to 10 if not provided
    ...
    # should return cursor as -1, when there is no more items in db to get, so frontend, should know not to make those requests anymore

    if "cursor" not in query_params: # case of the first pagination

        if RoommatePost.objects.all().count() < limit:
            roommatePosts = RoommatePost.objects.all()
            cursor = -1
        else:
        ...
        if RoommatePost.objects.filter(pk__gt=previous_cursor).count() < limit:
            roommatePosts = RoommatePost.objects.filter(pk__gt=previous_cursor)
            cursor = -1
        else:
