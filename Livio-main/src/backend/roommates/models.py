...
from thefuzz import fuzz # will be used for fuzzy search in the Search class
from thefuzz import process # will also be used for fuzzy search in the Search Class
...
class RoommatePost(Post):
    ...
    numberOfPeopleInterested: int = models.IntegerField(default=0) # this is the number of people who have added the roommate post to their interested
...
# will be a utility class for searching roommate posts based on the different keywords 
class Search:

    """
    Class Name: Search 
    Date of Code: November 19, 2025
    Programmer's Name: Arthur Lazaryan
    Description: Utility class for searching rommate posts
    """
    
    # will recieve a list of keywords to search from 
    # strict keywords search 
    def roommateKeywordSearch(keywords: list[str]):
        
        # go through each of the keywords in the search, and filter for 
        allRoommatePosts = RoommatePost.objects.all()
        allSize = allRoommatePosts.count()

        # figure out how terms will work for budget, moveindate, and more 
        for term in keywords:
            # check what attribute of roommate post it matches up with 
            # checks if the term matches with that of gender 
            if term in Gender.objects.values_list('name', flat=True): # will return a list rather than tuples
                if term == 'M':
                ...
                allRoommatePosts = allRoommatePosts.filter(profile__gender=genderType) # will filter only those in gender
                break
            if term in Nationality.objects.values_list('name', flat=True):
                allRoommatePosts = allRoommatePosts.filter(profile__nationality__name=term)
                break
            if term in Feature.objects.values_list('name', flat=True):
                allRoommatePosts = allRoommatePosts.filter(features__name=term)

            # check for the date and stuff 
        
        # check if any filtering had happened, if not return the empty list/queryset
        if allRoommatePosts.count() == allSize:
            return [] 
        else:
            return allRoommatePosts 

            



    # perform fuzzy search on roommate searchs 
    def fuzzyRoommateSearch(keywords: list[str]):

        for term in keywords:

            # do fuzzy search for all values in nationality
            pass
...
class Filter:
    ...
    # filter on the budget specificed on the post
    @staticmethod 
    def ageFilter(lowerBound: float, upperBound: float) -> list[RoommatePost]:
        posts = RoommatePost.objects.filter(profile__age__range=(lowerBound, upperBound))
        return posts 
    ...
    @staticmethod
    def featuresFilter(features: list[str]) -> list[RoommatePost]:
        
        # filteredRoommatePosts = [] # the amount of filtered roommate posts is currently an empty list
        filteredRoommatePosts = RoommatePost.objects.all()  # gets all of the roommate objects 

        # go through each feature, and filter based on it 
        for feature in features:
            filteredRoommatePosts = filteredRoommatePosts.filter(features__name=feature)# will query based on the title (will keep on querying and updating the queryset based on the filter shown)

        return filteredRoommatePosts 
